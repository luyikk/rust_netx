...
```rust
// Inside a server controller method — get another client's token and call its interface
...
async fn main() -> anyhow::Result<()> {
    ...
    client.init(ClientController).await;
...
Applied to an **`impl`** block. Automatically attaches `#[async_trait]` to the async trait implementation.
...
```toml
# Server
netxserver = { version = "2.2", default-features = false, features = ["use_rustls"] }
# Client
netxclient = { version = "2.2", default-features = false, features = ["use_rustls"] }
```
...
```toml
netxserver = { version = "2.2", default-features = false, features = ["use_openssl"] }
netxclient = { version = "2.2", default-features = false, features = ["use_openssl"] }
```
...
用于 **`impl`** 块上，自动为异步 trait 实现附加 `#[async_trait]`。
...
```rust
// 通过 token 获取客户端接口代理（call_peer 宏在服务端 prelude 中）
let peer = impl_ref!(token => IClient);
peer.on_message("hello".to_string()).await;
```
