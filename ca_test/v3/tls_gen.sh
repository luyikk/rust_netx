...
if [[ "s_$FUNCTION" == "s_client" ]] ; then
  ...
  if [[ -f $EXTCONF ]] ; then
    ...
    echo 'basicConstraints=CA:FALSE' >> $EXTCONF \ 
    echo '[ v3_ca ]' >> $EXTCONF \ 
    echo 'subjectKeyIdentifier=hash' >> $EXTCONF \ 
    echo 'authorityKeyIdentifier=keyid:always,issuer' >> $EXTCONF \ 
    echo 'basicConstraints = critical,CA:false' >> $EXTCONF \ 
    echo 'keyUsage = critical, digitalSignature, cRLSign, keyCertSign' >> $EXTCONF \
  ...
  -CAcreateserial -out $CRT -extensions v3_ca -extfile $EXTCONF
  rm -v $EXTCONF
